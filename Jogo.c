...
int main(int argc, char *argv[])
{
	...
	SDL_WM_SetCaption("Super DragonJewelled", NULL);
	TTF_Init();
...
void CarregaMusica()
{
	music=Mix_LoadMUS("sound/D.mp3");
	menu_music=Mix_LoadMUS("sound/musica.mp3");
	...
	grito=Mix_LoadMUS("sound/grito.mp3");
}
...
int BlitaPoses()
{
	...
	int GokuContPos=0;
		...
		Mix_PlayChannel( -1, ki, 0 );
		Mix_PlayChannel( -1, fala, 0 );
		while(running==1)
		{
			int FPS=10;
		  int t1 = SDL_GetTicks();
...
int Confronto2()
{
	...
	{	
	  ...
	  int t1 = SDL_GetTicks();
		...
		if(vegDest.x>=gokuDest.x-200 && vegDest.x<=gokuDest.x<=+30 )
			Mix_PlayChannel( -1,luta, 0 );
...
Mix_PlayChannel( -1, ki, 0 );
	...
	Mix_PlayChannel( -1, explosaofim, 0 );
	explosaofim=Mix_LoadWAV("sound/missile.wav");
	Mix_PlayChannel( -1, explosaofim, 0 );
	...
	SDL_Surface *text=NULL;
	TTF_Font *font = TTF_OpenFont("recorde.ttf",40);
	SDL_Color color={0,0,0}	;
	SDL_Rect rect={260,320};
	
	text = TTF_RenderText_Solid(font,"Super DragonJewelled!!",color);
...
void intro()
{
	...
	teleport=Mix_LoadWAV("sound/teleport.wav");
	luta=Mix_LoadWAV("sound/Luta.wav");
	ki=Mix_LoadWAV("sound/Ki.wav");
	voo=Mix_LoadWAV("sound/voo.wav");
	fala=Mix_LoadWAV("sound/fala.wav");
	hit=Mix_LoadWAV("sound/1hit.wav");
	explosaofim=Mix_LoadWAV("sound/1.wav");
